...
document.addEventListener('pointermove', event => { const drag = state.drag; if (!drag || drag.pointerId !== event.pointerId) return; const cursor = point(event); drag.node.x = snap(cursor.x - drag.dx); drag.node.y = snap(cursor.y - drag.dy); drag.group.setAttribute('transform', 'translate('+drag.node.x+','+drag.node.y+')'); updateWirePositions(); updateMetrics(); });
document.addEventListener('pointerup', event => { if (!state.drag || state.drag.pointerId !== event.pointerId) return; state.drag = null; renderNodes(); });
